...
> 다른 프로젝트의 룰·패턴·산출물을 참조할 때 사용. 노트북 이동 시 본 섹션만 갱신.

| 항목 | 경로 |
|---|---|
| 참고 프로젝트 루트 | `C:\WORK\PROJECT\Practice\ref-project` |

### 사용 규칙

- 사용자가 "GDI 룰 봐바", "참고 프로젝트의 XX 봐줘" 같이 말하면 위 경로 하위에서 찾는다
- 거기서 가져올 만한 룰/패턴이 발견되면, **rental-crm 도메인에 맞춰 변형 후 복사** 하는 게 원칙 (단순 경로 참조는 노트북 이동 시 깨짐)
- 복사한 룰은 출처를 본문에 명시 (예: `> 출처: 참고 프로젝트의 audit-columns.md (rental-crm 도메인에 맞춰 변형)`)

### 노트북 이동 시 체크리스트

1. 새 노트북에서 위 경로에 참고 프로젝트 폴더 존재 확인
2. 없으면 본 표의 경로 갱신
3. 변경 후 새 세션 시작 (현재 세션은 옛 컨텍스트로 동작 가능)
