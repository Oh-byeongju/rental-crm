...
erDiagram
    ...
    CM_ROLE ||--o{ CM_ROLE_MENU : "has"
    CM_MENU ||--o{ CM_ROLE_MENU : "assigned"
...
| CM_ROLE | 권한(역할) | 역할 정의 |
| CM_ROLE_MENU | 역할-메뉴 권한 | 역할별 메뉴 접근 권한 |
...
### CM_ROLE_MENU (역할-메뉴 권한)

| 컬럼명 | 타입 | NOT NULL | PK/FK | 설명 |
|---|---|---|---|---|
| ROLE_MENU_ID | NUMBER | Y | PK | 권한-메뉴 ID |
| ROLE_ID | NUMBER | Y | FK | CM_ROLE.ROLE_ID |
| MENU_ID | NUMBER | Y | FK | CM_MENU.MENU_ID |
| READ_YN | CHAR(1) | Y | | 조회 권한, 기본값 N |
| WRITE_YN | CHAR(1) | Y | | 등록/수정 권한, 기본값 N |
| DELETE_YN | CHAR(1) | Y | | 삭제 권한, 기본값 N |
